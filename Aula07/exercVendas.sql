...
--Encontre a descrição, a unidade dos produtos e, caso ele se relacione com itens do pedido, mostre a quantidade do produto nos itens.
SELECT p.descricao, p.unidade, ip.quantidade 
FROM produto p 
LEFT OUTER JOIN item_pedido ip ON p.codigo_produto = ip.codigo_produto;

--Repita a consulta acima para que apareça apenas os produtos que não estejam em um item pedido (usar a comparação IS NULL).
SELECT p.descricao, p.unidade, ip.quantidade 
FROM produto p 
LEFT OUTER JOIN item_pedido ip ON p.codigo_produto = ip.codigo_produto
...
SELECT p.descricao, p.unidade, ip.quantidade 
FROM produto p 
RIGHT OUTER JOIN item_pedido ip ON p.codigo_produto = ip.codigo_produto;

--Encontre a descrição, a unidade dos produtos e, caso ele se relacione com itens do pedido, mostre a quantidade do produto nos itens.
SELECT p.descricao, p.unidade, ip.quantidade 
FROM produto p 
FULL OUTER JOIN item_pedido ip ON p.codigo_produto = ip.codigo_produto;

INSERT INTO vendedor VALUES (555, 'Pedro Paulo', 1000, 'C');

--Vai haver uma demissão na empresa de Paulo Afonso. Ele pediu um relatório que informe a quantidade de pedidos que cada vendedor realizou. Os que não venderam nada serão demitidos.
SELECT v.nome_vendedor, pe.prazo_entrega
FROM vendedor v
LEFT OUTER JOIN pedido pe ON v.codigo_vendedor = pe.codigo_vendedor
WHERE pe.codigo_vendedor IS NULL;

SELECT v.nome_vendedor, COUNT(pe.codigo_vendedor)
FROM vendedor v
LEFT OUTER JOIN pedido pe ON v.codigo_vendedor = pe.codigo_vendedor
