...
--Selecione o nome de todos os vendedores que possuem pedidos com prazo de entrega maior que 15 dias.	
SELECT v.nome_vendedor FROM vendedor v
WHERE v.codigo_vendedor IN 
	(SELECT p.codigo_vendedor from pedido p
	 WHERE p.prazo_entrega > 15);

--Selecionar o nome dos vendedores que não venderam nenhum pedido.
SELECT v.nome_vendedor FROM vendedor v
WHERE v.codigo_vendedor NOT IN 
	(SELECT p.codigo_vendedor FROM pedido p);

--Utilizando Sub Consultas, pesquise o nome dos vendedores que venderam algum pedido para clientes de SP. 
SELECT v.nome_vendedor FROM vendedor v
WHERE v.codigo_vendedor IN
	(SELECT p.codigo_vendedor FROM pedido p
...
--Selecionar TODOS OS números dos pedidos feitos para vendedores com salários menor que 1000.
SELECT * FROM pedido p
WHERE p.codigo_vendedor = (SELECT v.codigo_vendedor FROM vendedor v
						   WHERE v.salario_fixo < 1000);

--Faça uma consulta que retorne o nome dos vendedores caso exista algum pedido com prazo de entrega igual a 15.
SELECT v.nome_vendedor FROM vendedor V
WHERE EXISTS 
	(SELECT * from pedido p
	 WHERE p.prazo_entrega = 15);

--1) Utilizando sub consultas, retorne o nome dos vendedores que venderam pedidos com prazo de entrega igual a 15 ou 20.
SELECT v.nome_vendedor FROM vendedor v 
WHERE v.codigo_vendedor IN
	(SELECT p.codigo_vendedor FROM pedido p
	 WHERE p.prazo_entrega = 15 OR p.prazo_entrega = 20);

SELECT v.nome_vendedor FROM vendedor v 
WHERE v.codigo_vendedor IN
	(SELECT p.codigo_vendedor FROM pedido p
	 WHERE p.prazo_entrega IN (15, 20));

--2) Repita a consulta 1 de outra maneira. 
